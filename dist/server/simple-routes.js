...
const payment_verification_1 = require("./payment-verification");
async function registerRoutes(app) {
    ...
    app.post("/api/auth/register", (req, res) => {
        try {
            ...
            const result = (0, simple_auth_1.registerUser)(username, password);
            if (result.success) {
                res.json({
                    user: result.user,
    ...
    app.post("/api/auth/login", (req, res) => {
        try {
            ...
            const result = (0, simple_auth_1.loginUser)(username, password);
            if (result.success) {
                res.json({
                    user: result.user,
                    message: result.message
    ...
    app.post("/api/auth/claim-daily-reward", (req, res) => {
        try {
            ...
            const result = (0, simple_auth_1.updateDailyRewardClaim)(username);
            if (result.success) {
                res.json({
                    user: result.user,
                    message: result.message
    ...
    app.post("/api/auth/update-username", (req, res) => {
        try {
            ...
            const result = (0, simple_auth_1.updateUsername)(userId, newUsername);
            if (result.success) {
                res.json({
                    user: result.user,
                    message: result.message
    ...
    app.post("/api/game/place-bet", (req, res) => {
        try {
            ...
            const result = (0, simple_auth_1.placeBet)(userId, betAmount);
            if (result.success) {
                res.json({
                    user: result.user,
                    message: result.message
    ...
    app.post("/api/game/win-bet", (req, res) => {
        try {
            ...
            const result = (0, simple_auth_1.winBet)(userId, betAmount, winnings);
            if (result.success) {
                res.json({
                    user: result.user,
                    message: result.message
    ...
    app.post("/api/game/lose-bet", (req, res) => {
        try {
            ...
            const result = (0, simple_auth_1.loseBet)(userId, betAmount);
            if (result.success) {
                res.json({
                    user: result.user,
                    message: result.message
    ...
    app.post('/api/verify-payment', async (req, res) => {
        try {
            ...
            const verificationResult = await (0, payment_verification_1.verifyPayment)({
                amount,
                walletAddresses,
                userId
            });
    ...
    function findSafeSpawnPosition(room) {
        ...
        const existingPlayers = Array.from(room.gameState.players.values()).filter((p) => p.segments && p.segments.length > 0);
